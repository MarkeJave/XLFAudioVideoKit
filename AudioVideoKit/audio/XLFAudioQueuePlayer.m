                            ...
                            AudioQueueBufferRef  inBuffer)
{
    ...
    if([[player audioData] count] > 0){
        ...
        if (audioData) {
            ...
            if([audioData length] > kDefaultOutputBufferSize){
                
                NSData *subData = [audioData subdataWithRange:NSMakeRange(kDefaultOutputBufferSize, [audioData length] - kDefaultOutputBufferSize)];
                [[player audioData] replaceObjectAtIndex:0 withObject:subData];
            }
            else{
                [[player audioData] removeObjectAtIndex:0];
            }
            memcpy(inBuffer->mAudioData, [audioData bytes], MIN(kDefaultOutputBufferSize, [audioData length]));
            inBuffer->mAudioDataByteSize = (UInt32)MIN(kDefaultOutputBufferSize, [audioData length]);
...
- (void)initSession
{
    ...
    sessionCategory = kAudioSessionCategory_PlayAndRecord;         //可在后台播放声音
    AudioSessionSetProperty(kAudioSessionProperty_AudioCategory, sizeof(sessionCategory), &sessionCategory);
...
//    AudioQueueSetParameter (_playQueue,kAudioQueueParam_Volume,gain);
    ...
    //开启播放队列
    [self setIsPlaying:!AudioQueueStart(_playQueue,NULL)];
}
